...
- Examples:
    ...
    - "Ponies spend more than 50 bits/day at the Crystal Empire fair" $\to$ $H_1: \mu > 50$.
    
    - "80% of ponies are satisfied with their visit to the fair" $\to$ $H_1: \pi = 0.80$.    
    
    - "The variability in daily pony spending is less than $100" $\to$ $H_1: \sigma^2 < 100$.

    - "The median wait time for a ride at the Ponyville Fair is greater than 30 minutes" $\to$ $H_1: M > 30$.
    
    - "Fewer than 25% of ponies purchase caramel apple treats" $\to$ $H_1: \pi < 0.25$.    
...
- Examples:
    ...
    - "Ponies spend more at the Crystal Empire fair than at the Ponyville fair" $\to$ $H_1: \mu_{\text{CE}} > \mu_\text{PV}$.

    - "The proportion of ponies who purchase VIP cloud-seats differs between season-pass holders and single-day ticket holders" $\to$ $H_1: \pi_\text{SP} \ne \pi_\text{SD}$.

    - "The difference in VIP cloud-seat purchase rates between season-pass holders and single-day guests is less than 10 percentage points" $\to$ $H_1: \pi_\text{SP} - \pi_\text{SD} < 0.10$.
...
- Once we have the alternative hypotheses, we know the null hypothesis is in the opposite direction.
    ...
    - "The difference in VIP cloud-seat purchase rates between season-pass holders and single-day guests is less than 10 percentage points"
        - $H_0: \pi_\text{SP} - \pi_\text{SD} \ge 0.10$
        - $H_1: \pi_\text{SP} - \pi_\text{SD} < 0.10$
...
$p$-value
:::
The **p*-value** is the probability of observing the current test statistic or one more extreme.
:::
